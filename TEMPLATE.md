# Template per Note - Second Brain JS

> **Nome concetto**: [Inserire nome]  
> **Categoria**: [01-fondamenti / 02-funzioni / etc.]  
> **Data creazione**: [GG/MM/AAAA]  
> **Stato**: [ ] Draft / [→] In revisione / [✓] Completo

---

## 📝 Descrizione

[Spiegazione concisa del concetto (2-3 frasi)]

## 🎯 Concetti Chiave

- **Punto chiave 1**: Spiegazione
- **Punto chiave 2**: Spiegazione
- **Punto chiave 3**: Spiegazione

## 💻 Esempi di Codice

### Esempio Base

```javascript
// Codice di esempio base
```

### Esempio Avanzato

```javascript
// Codice di esempio più complesso
```

### Caso d'Uso Pratico

```javascript
// Esempio di utilizzo nel mondo reale
```

## ⚠️ Gotcha / Errori Comuni

- ❌ **Errore comune 1**: Cosa evitare
- ❌ **Errore comune 2**: Cosa evitare

## ✅ Best Practices

- ✓ **Best practice 1**: Consiglio
- ✓ **Best practice 2**: Consiglio

## 🔗 Collegamenti

**Prerequisiti** (concetti da conoscere prima):

- [[concetto-collegato-1]]
- [[concetto-collegato-2]]

**Concetti Correlati**:

- [[concetto-correlato-1]]
- [[concetto-correlato-2]]

**Approfondimenti** (concetti successivi):

- [[concetto-avanzato-1]]
- [[concetto-avanzato-2]]

## 📚 Riferimenti

- [MDN - Nome Concetto](link)
- [JavaScript.info - Capitolo](link)
- Altre risorse utili

## 📌 Note Personali

[Spazio per riflessioni, domande, applicazioni personali]

---

**Tags**: `#javascript` `#categoria` `#sottocategoria`
